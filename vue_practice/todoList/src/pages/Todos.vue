  ...
  export default {
    ...
    setup() {
      ...
      /* const addTodo = (todo) => {//add-todo로 넘긴값 todo로 받음
        error.value = '';  
        axios.post('http://localhost:5001/todos', {//위치, 작업
      ...
      const addTodo = async(todo) => {
        error.value = '';
        try{
          const res = await axios.post('http://localhost:5001/todos',{
            subject:todo.subject,
            completed:todo.completed
          })
          todos.value.push(res.data)      
      ...
      const getTodos = async(params)=>{
        try{
          const res = await axios.get('http://localhost:5001/todos');//넘어오는 값없음 접속만
      ...
      const toggleTodo = async(idx) => {
        ...
        const id = todos.value[idx].id
        try{
          await axios.patch('http://localhost:5001/todos/' + id,{
