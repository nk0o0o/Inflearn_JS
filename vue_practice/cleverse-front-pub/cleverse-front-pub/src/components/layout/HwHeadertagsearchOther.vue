					...
					<div class="tag">
						<form action="" :class="{'hid':isHid}">
							<div class="search-type3">
								...
								<div class="list scroll" v-show="hasIpt">
...
export default defineComponent({
  components: {
	...
	setup() {
		...
		const tabsType5 = ref(['태그검색', '이름검색']);
		const iptLen = ref('');
		const tagList = ref(['Photoshop', 'Sketch', 'UI 디자이너']);
		const isTagSearch = ref(false);
		const isSticky = ref(false);
		const isHidden = ref(true);
		const isHid = ref(true);
		const hasIpt = computed(() => iptLen.value.length > 3)
		return {
			currentTab,
			tabsType5,
			iptLen,
  ...
  methods: {
		inputView(e :any){
			this.isHid = false
			// let target = e.target;
			// target.parentElement.previousElementSibling.classList.remove('hid');
			// target.parentElement.classList.add('hid');
			// this.tagList.splice(0, this.tagList.length);
			// if(this.tagList.length <= 0){
			// 	this.isTagSearch = false;
			// }
			// this.$emit('update', this.isTagSearch);
		},
		update(){
