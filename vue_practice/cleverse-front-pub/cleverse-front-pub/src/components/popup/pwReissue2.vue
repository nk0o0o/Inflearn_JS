<template>
  <base-popup
    ...
    title="비밀번호 재발급 안내"
    class="ly-pop-type1 pwreissue certification"
  >
    <template v-slot:body>
      ...
      <div class="input-box type5">
          <input type="text" required placeholder="인증번호">
          <button type="reset" class="input-txt-del"><span class="blind">입력 삭제</span></button>
          <span class="certification-time">02:18</span>
          <div class="error" v-if="error">에러 문구</div>
      </div>
    </template>
    ...
    <template v-slot:btnWrap>
      <div class="btn-wrap">
        <v-button class="btn biggest line-type1">인증번호 재발송</v-button>
        <v-button class="btn biggest">인증</v-button>
      </div>
    </template>
  </base-popup>
</template>
